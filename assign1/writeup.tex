...
\ifnum\me<2
\begin{solution}
\\We performed experiments on the MNIST dataset trying to classify on it using a 4 layered network. We achieved an accuracy of $99.36\%$ on the test set with our model. A detailed explanation of our experiments is mentioned below:
\begin{enumerate}
...
\\The model starts with a convolution layer of $5 \times 5$ filter bank of size $32$. This is followed by a ReLu layer and a pooling layer. This is followed by another convolution layer of $5 \times 5$ filter bank of size $64$ which is followed by another set of ReLu and pooling layers. Then it is followed by a linear layer mapping to 1024 points which is terminated by a dropout layer and another linear layer resulting in 10 outputs. Using the conventions defined in \cite{MCDNN} we can write it as $1 \times 32 \times 32 - 32C5 - ReLU - MP2 - 64C5 - ReLU - MP2 - 1024N - ReLU - Dropout - 10N$.
\\The Loss function we used was the negative log likelihood measure.

\item Training and distortions:
\\While training we make use of the ADAM optimizer \cite{ADAM} for the stochastic gradient based optimization. The hyperparameters used for ADAM are: learning rate $\alpha = 0.001$, exponential decay rates $\beta_1 = 0.9, \beta_2 = 0.999$ and the fuzz factor $\epsilon = 10^{-8}$. At each epoch we also introduce some distortions to increase the number of training images. This allows for training the network with many weights, making them insensitive to in-class variability. The distortions introduced are a combination of rotation, shear and translation with the following parameters:
\begin{enumerate}
...
\item Translation of $[-2,2]$ pixels in either direction.
\end{enumerate}
\item Testing:
\\At each epoch of training the model was tested on the validation set. This was done to identify the stopping criteria. The early stopping criteria we use is one of a patience based models. We start with an initial patience level of 20. Then at each epoch , we check to see if the accuracy on the cross-validation set has increased compared to the best model seen earlier. If the improvement is $>1\%$ then we set the patience to twice the current epoch (if not higher already). If the improvement is $<1\%$ then we increase the patience by 2. The training stops when number of epochs = patience. This way we implement an early stopping preventing unnecessary runtime and overfitting.
\end{itemize}
...
As mentioned earlier our model gave an accuracy of $99.36\%$ on the test set. The confusion matrix on the test set is as follows:
\resettabtoks
\foreach \i in {1,...,10}
{
	\addtabtoks{& \textbf}
...
& \textbf{Accuracy}
\\\hline
\textbf{1}  & 977 &    0 &    0 &    0 &   0 &   0 &   0 &    0 &   2 &    1 & 99.694\% \\\hline	
\textbf{2}  &   0 & 1134 &    0 &    0 &   0 &   0 &   0 &    1 &   0 &    0 & 99.912\% \\\hline	
\textbf{3}  &   0 &    1 & 1026 &    1 &   0 &   0 &   0 &    2 &   2 &    0 & 99.419\% \\\hline	
\textbf{4}  &   0 &    0 &    0 & 1007 &   0 &   2 &   0 &    0 &   1 &    0 & 99.703\% \\\hline	
\textbf{5}  &   0 &    0 &    0 &    0 & 970 &   0 &   3 &    1 &   1 &    7 & 98.778\% \\\hline	
\textbf{6}  &   0 &    0 &    0 &    6 &   0 & 885 &   1 &    0 &   0 &    0 & 99.215\% \\\hline	
\textbf{7}  &   2 &    1 &    0 &    1 &   1 &   4 & 947 &    0 &   2 &    0 & 98.852\% \\\hline	
\textbf{8}  &   0 &    3 &    4 &    0 &   1 &   1 &   0 & 1016 &   0 &    3 & 98.833\% \\\hline	
\textbf{9}  &   0 &    0 &    1 &    0 &   0 &   1 &   2 &    0 & 968 &    2 & 99.384\% \\\hline	
\textbf{10} &   0 &    0 &    0 &    0 &   2 &   1 &   0 &    0 &   1 & 1005 & 99.604\% \\\hline	
\end{tabular}
\end{table}
As we can see from the above table the most confusion happens for cases such as 8 being mistaken for 3 or 0, 7 being mistaken for 1, 6 being mistaken for 4 etc, which are sometimes hard for humans to discern too. Below are the images from the test set that were misclassified.
\begin{figure}[H]
\foreach \x in {1,...,65}
{
	\includegraphics[scale=1]{mistake\x.jpg}
}
\caption{Set of images in test set on which mistakes were made}
\end{figure}
As we can see most of these are hard for humans to correctly identify as well. Overall this model has given good results on the MNIST dataset.
\end{enumerate}
\end{solution}
