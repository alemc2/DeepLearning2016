...
cmd:option('-epoch_step', 200, 'epoch step')
   ...
   function DataAugment:updateOutput(input)
      if self.train then
         ...
         for i=1,bs do
            ...
            local rshift = torch.uniform(-30/255,30/255)
            local gshift = torch.uniform(-30/255,30/255)
            local bshift = torch.uniform(-30/255,30/255)
            ...
            local theta = torch.uniform(-math.pi/10, math.pi/10)
            -- Translation parameters +/- 10 pixels
            local tx = torch.random(-15,15)
            local ty = torch.random(-15,15)
            -- Scaling parameters 0.8-1.2 times. Also does strech when aspect ratio is lost
            local sx = (((torch.rand(1)-0.5)*0.225)+1)[1]
            local sy = (((torch.rand(1)-0.5)*0.225)+1)[1]
            ...
            --vignette(input[i],torch.uniform(0,0.8))
