...
function Provider_Second:__init(group)
  local group = group or error('missing group argument')
  local trsize = 100000
  local channel = 4
  local height = 10
  local width = 10

  local allData = torch.load('stl-10/secondlevel.t7')

  self.secondData = {
     data = allData[group]:clone(),
